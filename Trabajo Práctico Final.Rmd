...
Podemos ver que las **"Directrices de Gestión"** es el programa más certificado de todos con 1083 casos, mientras que el **"Premio Nacional a la Calidad"** el que menos, con 66 casos.
...
**Alojamiento** (1237), seguido, aunque por bastante diferencia, de **Gastronomía** (448) y **Agencia de viaje** (364) son los rubros más frecuentes de nuestra tabla.

Por último, veamos la distribución geográfica de nuestras iniciativas.

```{r}
pais <- certificaciones %>% 
  group_by(pais) %>% 
  summarise(N=n()) %>% 
  arrange(desc(N)) %>% 
  print(n=Inf)

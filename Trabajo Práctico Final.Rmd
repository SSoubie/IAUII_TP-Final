...
El cambio climático es uno de los desafíos más urgentes que enfrenta la humanidad. Se espera que en los próximos 15 años, el incremento de la temperatura media mundial por encima de los 1,5ºC. lleve al aumento en la periodicidad e intensidad de los fenómenos climáticos extremos, acarreando grandes consecuencias para toda la sociedad (particularmente, para los sectores más vulnerables), poniendo en duda varios de los avances económicos y sociales logrados hasta el momento. 
En este marco, es necesario promover una transición hacia un modelo de desarrollo más sostenible y fomentar aquellas iniciativas que conlleven un uso más racional de los recursos, alineados con los Objetivos de Desarrollo Sustentable planteados por la ONU para el 2030.

Frente a esta situación, distintos países, ciudades e industrias han iniciado el camino hacia una proyección sostenible, entre las que podemos destacar el sector turístico, que moviliza diversas una economía creciente en diversos escenarios, naturales y artificiales. 
...
Con ello en vista, el siguiente trabajo se propone hacer un aporte a la discusión sobre el nivel actual de la transición verde en el sector turístico, y en nuestro análisis, centrándonos en los hoteles nacionales. 
Con tal fin, se realiza un análisis de los establecimientos locales certificados con las directrices ambientales otorgadas por el Ministerio de Turismo y Deportes de la Nación, a partir de datos obtenidos del Ministerio de Turismo y Deopórte de la Nación, El mismo se encuentra disponible en <http://datos.yvera.gob.ar/>.

Dividiremos el trabajo en tres partes:
## 1º PARTE: Directrices de calidad ambiental
## 2º PARTE: Diagnostico
## 3º PARTE: Conclusiones

Particularmente, buscamos responder:
...
A continuación, analizaremos con mayor profundidad nuestro dataset. Para ello, nuestro primer paso consiste en "limpiar" nuestra base de datos.

Primero veamos las organizaciones registradas en nuestro dataset:
...
Vemos que algunas categorías repiten las mismas temáticas. 
Las agruparemos.
...
```{r}
certificaciones %>% # HAY QUE VER COMO HACER PARA ORDNER POR GRUPO Y POR SUBGRUPO
...
Podemos ver que todas las provincias del país cuentan con al menos una organización certificada en el Sistema Argentino de Calidad Turística. En este contexto, **Buenos Aires** es el distrito que cuenta con el mayor número de certificaciones (306), seguida de **Mendoza** (298).
**La Pampa** (3) y **Formosa** (7), en ese orden, son las que menos certificaciones tiene.


```{r}
Argentina <- st_read("https://infra.datos.gob.ar/catalog/modernizacion/dataset/7/distribution/7.13/download/departamentos.geojson")
```


```{r}
Provincias <- st_read("data/ign_provincia.shp")
```
...
```{r}
ambientales_ano_rubro_3 <- ambientales_ano_rubro %>% 
...
```{r}
ambientales_ano_rubro_3 <- ambientales_ano_rubro_3 %>% 
  select(-12) %>% 
  select(-(16:17))
```

```{r}
class(ambientales_ano_rubro_3$`Actividades recreativas y deportivas`)
```
```{r}
class(ambientales_ano_rubro_3$ano_distincion)
```
```{r}
library(janitor)
```
...
```{r}
ambientales_ano_rubro_4 <- ambientales_ano_rubro_3%>% 
  mutate(ano=apply (2, which.max))
```

## 3º PARTE: Conclusiones

A partir de los datos obtenidos en el siguiente trabajo, podemos concluir que queda mucho por seguir haciendo en términos de organismos tendientes a la sustentabilidad en la industria del turismo en todo el país. La necesidad de un seguimiento constante, incentivos con las certificaciones, sería un posible camino para que la industria del turismo se alinee cada vez más a objetivos que afecten menos al medioambiente y demuestren su operación tendiente a la sustentabildiad.
